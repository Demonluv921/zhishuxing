...
// 可选:从 supabase-config.json 注入云端配置(构建时生成)
let supabaseUrl = '';
let supabaseAnon = '';
const configPath = path.join(root, 'supabase-config.json');
if (fs.existsSync(configPath)) {
  try {
    const cfg = JSON.parse(fs.readFileSync(configPath, 'utf8'));
    supabaseUrl = cfg.url || '';
    supabaseAnon = cfg.anonKey || '';
    console.log('[Build] 已注入 Supabase 配置:', supabaseUrl);
  } catch (e) {
    console.warn('[Build] supabase-config.json 解析失败,以本地模式构建');
  }
}
...
const supabaseConfig = fs.readFileSync(path.join(src, 'supabase-config.js'), 'utf8');
const cloudSync = fs.readFileSync(path.join(src, 'cloud-sync.js'), 'utf8');
...
html = html.replace('__SUPABASE_CONFIG__', supabaseConfig
  .replace('__SUPABASE_URL__', JSON.stringify(String(supabaseUrl || '')))
  .replace('__SUPABASE_ANON__', JSON.stringify(String(supabaseAnon || ''))));
html = html.replace('__CLOUD_SYNC__', cloudSync);
